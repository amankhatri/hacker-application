...
require 'uri'
require 'geocoder'
class CurrentMeeting < ActiveRecord::Base
...
# belongs_to :meeting     ## in database but not used
  ...
  def self.search(params, options={}) #(search, page)
    ...
    order = params[:speakers] || "start_time,region.name,section.name"
    ...
    if ((params['geo_distance']) && (!params['geo_distance'].blank?) &&
      ...
      distance_calc = near_scope_options[:speakers].gsub(" ASC", "")
...
=end
    ...
    @meetings = Meeting.paginate(
      :page=>page, :per_page=>per_page, :speakers=>order,
